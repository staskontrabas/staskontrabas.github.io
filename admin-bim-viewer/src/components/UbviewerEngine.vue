...
import {UBWebLoader} from '@/utils/UBWebLoader'
...
export default {
    ...
    methods: {
        ...
        onSection(data){
        console.log('onSection 0', data)
            if(data.sts){
                // this.UBViewer.createSectionBox()
                switch(data.value){
                    case 'planex':
                    console.log('onSection', data)
                        this.UBViewer.createSectionPlaneX()
                        break
                    case 'planey':
                        this.UBViewer.createSectionPlaneY()
                        break
                    case 'planez':
                        this.UBViewer.createSectionPlaneZ()
                        break
                    case 'level':
                        this.UBViewer.createSectionBox()
                        break
                    case 'plane':
                        this.UBViewer.createSectionPlaneByLevelId(data.id)
                        break
                }
            }
            else{
                this.UBViewer.removeSectionPlane()
            }
        },
        onComputeRepeat(d){
        ...
        onSetColorForObjects(d){
            ...
            this.UBViewer.setColorForObjects(list, d.value.color, 0)
        },
        ...
        onDelta(d){
            this.UBViewer.clear()
            if(!d.value){
                let model = this.modelName[this.modelIds[0]]
                // this.UBViewer.loadModel(model.name, model.path, model.pathJson)
                this.UBViewer.loadSceneFromFile(model.path + model.name)
            }
            else{
                        ...
                        return new Promise((resolve) => {
                            ...
                            .then(pathJson => {
                                ...
                                // }
                                    ...
                                    this.UBViewer.alignEntities()
                                    this.UBViewer.displayScene()
                                resolve(true)
                            })
                        })
                    })
        ...
        getDeltaJson(){
            return new Promise(resolve => {
                let delta = this.modelIds[0]
                if(!this.deltaJson){
                    this.$store.dispatch('getFile', {
                        id: delta,
                        url: delta + '/delta.json',
                        token: this.token,
                        typeBlob: 'blob'
                    })
                    .then(res => {
...
// return
        ...
        onLoadModel(val){
        console.log('onLoadModel val', val)
            const filename = val.split('/').slice(-1)
            console.log('onLoadModel filename, this.modelLine', filename, this.modelLine)
            this.modelLine = this.modelLine.filter(mdl => mdl != filename)
            console.log('onLoadModel this.modelLine', this.modelLine)
            if(!this.modelLine.length){
            console.log('onLoadModel this.fileTemp', this.fileTemp)
                if(this.fileTemp.length){
                    let list = []
                    let file = this.fileTemp.pop()
                    if(file){
                        list.push(file)
                        this.loadModelToWasm(list.map(item => item.file))
                    }
                }
                else{
                console.log('onLoadModel this.fileList', this.fileList)
                    this.fileList.map((file, i) => {
                        // loadUBViewSceneFromBinData(file.id, file.obj.data.buffer, this.store)
                        let ubwebLoader = new UBWebLoader(this.store)
                        ubwebLoader.loadFromFileData(file.id, file.obj.data.buffer)

                        window.parent.postMessage({
                            action: 'preloaderStep',
                            title: 'Загрузка свойств модели'
                        }, "*")

                        console.log('onLoadModel i', i)
                        console.log('onLoadModel this.store', this.store)
                ...
                window.parent.postMessage({
                    action: 'preloaderStep',
                    title: 'Загрузка файлов модели '
                }, "*")
            }
        },
        loadModelToWasm(files){
            let promiseAll = []

                // console.log('loadModelToWasm files', files)
            files.map(file => {
                // console.log('loadModelToWasm file', file)
                if(file){
                    promiseAll.push(new Promise(resolve => {
                        let url = file.id

                        if(file.format != 'obj' && file.format != 'zip' && file.format != 'ubweb'){
                            if(file.tags.ubweb){
                                url += '/ubweb.zip'
                            }
                            else{
            ...
            .then(res => {
                ...
                // let objectURL = URL.createObjectURL(elem.file)
                // a.href = objectURL
                // a.download = 'ubweb.zip'
                // document.body.appendChild(a)
                // a.click()
                // URL.revokeObjectURL(objectURL)
                // a.remove()
                    JSZip.loadAsync(elem.file)
                    .then(zip => {
                        let filesList = Object.keys(zip.files)
                        return Promise.all(this.aqq(filesList, zip))
                    })
                    ...
                    .then(res => {
                        return new Promise((resolve) => {
                            ...
                            this.modelLine.push(res.obj.name)
                            this.createFile(res.obj)
                            resolve(res)
                        })
                    })
                    .then(res => {
                        this.modelName[elem.id] = {
                            name: res.obj.name,
                            path: res.obj.path,
                            pathJson: ''
                        }
                        this.fileList.push({obj: res.obj, id: elem.id})
                        // this.UBViewer.loadModel(res.obj.name, res.obj.path, '')
                        this.UBViewer.loadSceneFromFile(res.obj.path + res.obj.name)
                    })
                })
            })
        },
        loadModel(v){
            ...
            this.modelIds = [...this.modelIds, ...v.listId]
            this.token = v.token
            ...
            const getFile = (files) => {
                ...
                .then(res => {
                    res.map(elem => {
                        ...
                        .then(res => {
                            // console.log('loadModel zip aqq res', res)
                            return new Promise((resolve) => {
                                if(res.files.length){
                                    res.files.map(file => {
                                        this.createFile(file)
            ...
            .then(res => {
                this.fileTemp = res
                    .filter(fil => fil.status == 'fulfilled')
                    .map(obj => obj.value)
                this.fileCount = this.fileTemp.length
                // console.log('promiseAll', this.fileTemp)
                let list = [this.getFile()]
        ...
        aqq(filesList, zip){
            let list = []
            filesList.map(i => {
                // console.log('filesList i', i)
                if(i.split('.').pop() === 'obj' || i.split('.').pop() === 'ubweb'){
        ...
        set2dViewMode(data){
            this.UBViewer.set2dViewMode(data.value)
        },
        setOnPartialShowPercentCallback(val){
            window.parent.postMessage({
                action: 'partialEvent',
                event: 'percent',
                value: val
        ...
        setOnPartialShowBeginCallback(){
            window.parent.postMessage({
                action: 'partialEvent',
                event: 'start'
            }, "*")
        },
        setOnPartialShowEndCallback(){
            window.parent.postMessage({
                action: 'partialEvent',
                event: 'end'
            }, "*")
        ...
        receiveMessage(e){
            if(Object.prototype.hasOwnProperty.call(e.data, 'action')){
                switch(e.data.action){
                    ...
                    case 'set2dViewMode': this.set2dViewMode(e.data)
                            break
        ...
        initEngine(){
            if(this.UBViewer === null){
                ...
                new UBViewer({
                    ...
                    // this.UBViewer.setOnModelLoadedCallback( // старая
                    //     v => this.onLoadModel(v)
                    // )
                    this.UBViewer.setOnFileLoadedCallback( // новая
                        v => this.onLoadModel(v)
                    )
                    ...
                    // this.UBViewer.setOnPartialShowPercentCallback(
                    //     val => this.setOnPartialShowPercentCallback(val)
                    // )
                    // this.UBViewer.setOnPartialShowBeginCallback(
                    //     () => this.setOnPartialShowBeginCallback()
                    // )
                    // this.UBViewer.setOnPartialShowEndCallback(
                    //     () => this.setOnPartialShowEndCallback()
                    // )
    ...
    mounted(){
        ...
        // this.worker = new SharedWorker(window.location.origin + '/js/wubviewer.js')
        // worker.port.start()
        // console.log(window.location.origin, this.worker)
