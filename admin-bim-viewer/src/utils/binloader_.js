...
class BinLoader {
    ...
    loadProperties() {
        ...
        this.loadArray(() => {
            ...
            this.loadArray(() => {
                ...
                if (property.ValueIdx !== -1)
                    property.ValueIdx = this.propertyStrings[valueIdx];

                if (property.UnitsIdx !== -1)
                    property.UnitsIdx += this.propertyStrings[unitsIdx];
    ...
    groupByTitle(list) {
        return list.reduce(function (accumulator, currentValue) {
            (accumulator[currentValue.Title] = accumulator[currentValue.Title] || []).push(currentValue);
            return accumulator;
        }, {});
    };
}
