...
const actions = {
    getFileMeta({rootState, state, commit}, payload){
        ...
        return new Promise((resolve) => {
            api.get(http['cloud'], api['projects'] + 'document/' + file_id + '/meta' + access_query, {}, {
    ...
    setFileMeta({rootState, state, commit}, pld){
        ...
        return new Promise((resolve) => {
            api.put(http['cloud'], api['projects'] + 'document/' + file_id + '/meta', body, {
                authorization: 'Bearer ' + token,
            })
            .then(res => {
                resolve(res)
    ...
    setMeta({rootState, state, commit}, pld){

        return new Promise((resolve) => {
            if(pld.meta.collisions){
                commit('setMeta', pld.meta)
                resolve(pld.meta)
            }
        })
    },
    ...
    getFileInfo({rootState, commit}, payload){
        let token = rootState.auth.access_token
        let access_query = payload.access_query ? '?access=' + payload.access_query : ''
        // console.log('common get file inf pay', payload)

        return new Promise((resolve, reject) => {
            // + '/info'
            api.get(http['cloud'], api['getfile'] + payload.id + access_query, {}, {
                authorization: 'Bearer ' + token,
            })
            .then(res => {
                resolve(res)
