...
return
        ...
        hiddenOptionsBtnInit(){
            ...
            // TODO: отфильтровать опции по новым разрешениям, в $store.state.administration.user.permissions
            if(this.listModel.length == 1){
                this.options.delta.hidden = !this.deltaMode
            }
        },
        onDeltaWorker(data){
            // this.worker.postMessage({
        ...
        onDelta(v){
            ...
            let url = ''
            this.listModel[0].files.map(fl => {
                if(fl.version == this.getVersionFile(this.listModel[0]) - 1){
                    url = fl.url
                }
            })
            this.postMessage({
        ...
        receiveMessage(e){  // сообщения фрейма с вьюером
            if(Object.prototype.hasOwnProperty.call(e.data, 'action')){
                switch(e.data.action){
                    case 'ready':   // готовность вьюера во фрейме
    ...
    created(){
        ...
        const delta = parseInt(urlParams.getAll('delta') || 0)
        const docs = urlParams.getAll('doc')
        this.files = files
        this.deltaMode = !!delta

        let listPromises = []
        files.map(file => {
            listPromises.push(this.$store.dispatch('common/getFileInfo', {id: file + '/info'}))
        })
        Promise.all(listPromises)
        .then(res => {
            this.listModel = res
            let name = res.map(file => file.name).join(', ')
            this.$store.commit('toolbar/setDctView', {name: name, status: ''})
            this.hiddenOptionsBtnInit()
            this.listModel.map((file, ind) => {
                const doc = this.$store.state.workflow.listDocs.find(dct => dct.id == docs[ind])
                this.docMap[file.id] = doc
            })
            // this.model = this.$store.state.workflow.listDocs.find(dct => dct.id == doc)
            // console.log('this files info', res)
        })
        .catch(er => {
    ...
    mounted(){
        ...
        // console.log('user perms',this.$store.state.administration.user.permissions)
