<template>
    <v-card class="m-overhead m-overhead--white d-block pa-4 overflow-auto">
        ...
        <!-- file attachment section; one-way action atm, can't remove file association -->
        <div class="d-flex align-center">
            <v-checkbox
            :disabled="!checkOwner()"
            v-show="!docid"
            class="m-text--color-4a5568 m-text--size16"
            :ripple="false"
            color="black"
            v-model="needsFile"
            :label="needsFile ? 'Выберите файл:': 'Ассоциировать задачу с файлом?'"
            />
        </div>

        <!-- folders map for file attachment -->
        ...
        <!-- associated file -->
        <div class="d-flex align-center">
            <span class="m-text--color-4a5568 m-text--size14" v-show="!!docid">
                Ассоциированный файл: &nbsp; {{ dname }}
            </span>
        </div>
...
import GetFoldersForProject from "./GetFoldersForProject.vue"

export default {
    name: "TaskCreate",
    props: ["item","projectFoldersMap"],
    data() {
        return {
            name: this.item.name || "",
            selectStatus: this.item.status, //Sh: fixed - || null removed; otherwise tasks from category 0 lose status
            statusList: [
                { name: "Новая", value: 0 },
    ...
    methods: {
        ...
        sendTask() {
            ...
            if (this.dname != this.item.dname) {
                body.dname = this.dname;
            }
            if (this.docid != this.item.docid) {
                body.docid = this.docid;
            }
    ...
    components: { GetFoldersForProject }
}
</script>
