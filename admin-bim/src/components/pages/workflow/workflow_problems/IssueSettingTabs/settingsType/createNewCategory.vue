...
<template>
    <v-dialog 
        ...
        <div class="category-create-header">
            <span> Создать категорию проблем </span>
            <v-btn
                text
                icon
                @click="cancel"
...
export default {
    ...
    methods: {
        ...
        create() {
            if (!this.$refs['create_category_form']) {
                return null
            }
            if (!this.$refs['create_category_form'].validateForm()) {
                return null
            }
...
::v-deep .issue-create-category {
    ...
    --issue-create-form-height: calc(50vh - var(--issue-create-header-height) - var(--issue-create-footer-height));
 }
