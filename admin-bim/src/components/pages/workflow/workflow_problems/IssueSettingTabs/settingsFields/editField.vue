    ...
    <v-navigation-drawer
        ...
        <template>
            <div class="edit-problem-wrapper">
                ...
                <div class="edit-problem-header" style="margin-bottom: 16px;">
                    <div class="edit-problem-title-field" >
                        <span class="edit-problem-title-plate" style="font-size: 18px;">
                        ...
                        <el-tab-pane label="Подробности" name="details">
                            <div class="edit-problem-button-panel">
                                ...
                                <el-button class="remove-margin-right" type="default">
                                    <div class="el-btn-wrap delete" @click="deleteField">
            ...
            <div class="edit-problem-dialogue-wrapper">
                ...
                <div class="epd-body">
                    <span style="text-align: center; display: table-cell;">
                        Вы собираетесь удалить 1 поле. Вы уверены? 
                    </span>
                </div>
    ...
    methods: {
        ...
        saveChanges() {
            ...
            new_field.value = {
                ...
                value: source.value_type == 'list' 
                        ? makeListFromObject({
                            list: source.values_list,
                            value: source.values_list[0],
                        }) 
                        : source.selected_value
            }
            new_field.visible = source.visible
            
            new_field.type_id = source.type_id

            function makeListFromObject(obj) {
                return [obj.value ? obj.value : '', ...obj.list]
            }

            console.log('updating field', new_field)
            this.$emit('save', new_field)
        },
        checkLength() {
            const source = this.form_data
            if (source !== null && source.value_type == 'list' && source.values_list.length == 0) {
                return false
            }
            return true
        }
